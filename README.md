...
Для тестирования я настроил фильтрацию. Логика в том, что если в коде сервиса или в самих тестах произойдут изменения, то первый этап с тестами для конкретного сервиса должен отработать. 
...
Он работает как сервис на локальном компьютере в фоновом режиме и постоянно прослушивает команды со стороны GitHub посредством веб-сокетов (отправка логов на сервер) и по технологии Long Polling (ожидание задач).
