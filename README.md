...
Для тестирование я настривал фильтрацию. Логика в том, что если в коде сервиса или на самих тестах произойдут изменения, то первый этап для конкретного сервиса должен отработать.
Если же изменений не произошло, то этап выполнения тестов и сборки сервиса проигнорируется. Это сильно экномит время и ресурсы в ходе выполнения разработки. Нет смысла пересобирать или проверять то что не трогали, то что и так работает.
На этом этапе когда создаются образы я так же настроил фильтры, но из фильтрации убрал тесты. При развёртывании и сборке мне нужны только чистые сервисы, которые уже прошли стадию тестирования и в каторых не выявлено ошибок.
Этап сборки Docker-образов автоматически запускается при слиянии изменений из рабочей ветки в главную ветку main. Затем запускается сборка и развёртывание приложения на моём локальном докере.
